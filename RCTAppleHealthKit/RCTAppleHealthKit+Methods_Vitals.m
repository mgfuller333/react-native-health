...

{
    ...
    HKSampleQuery *query = [[HKSampleQuery alloc]
        ...
        resultsHandler:^(HKSampleQuery *query, NSArray *results, NSError *error) {
            ...
            NSMutableArray *data = [NSMutableArray arrayWithCapacity:(10)]; // What does this 10 mean? (is it too small?)

            for (HKQuantitySample *sample in results) {
            NSLog(@"sample for LowHeartRateEvent %@", sample);
...

{
    ...
    HKSampleQuery *query = [[HKSampleQuery alloc]
        ...
        resultsHandler:^(HKSampleQuery *query, NSArray *results, NSError *error) {
            ...
            for (HKQuantitySample *sample in results) {
            NSLog(@"sample for HighHeartRateEvent %@", sample);
            NSString *startDateString = [RCTAppleHealthKit buildISO8601StringFromDate:sample.startDate];
            NSString *endDateString = [RCTAppleHealthKit buildISO8601StringFromDate:sample.endDate];
...

{
    ...
    HKSampleQuery *query = [[HKSampleQuery alloc]
        ...
        resultsHandler:^(HKSampleQuery *query, NSArray *results, NSError *error) {
            ...
            for (HKQuantitySample *sample in results) {
            NSLog(@"sample for IrregularHeartRhythmEvent %@", sample);
            NSString *startDateString = [RCTAppleHealthKit buildISO8601StringFromDate:sample.startDate];
            NSString *endDateString = [RCTAppleHealthKit buildISO8601StringFromDate:sample.endDate];
...
{
    HKQuantityType *WalkingSteadiness = [HKQuantityType quantityTypeForIdentifier:HKQuantityTypeIdentifierAppleWalkingSteadiness];

    HKUnit *unit = [HKUnit percentUnit]; // double check this
    ...
    [self fetchQuantitySamplesOfType:WalkingSteadiness //
                                unit:unit
                           predicate:predicate
                           ascending:ascending
                               limit:limit
        ...
        if(results){
            ...
            NSLog(@"error getting WalkingSteadiness percent samples: %@", error);
            callback(@[RCTMakeError(@"error getting WalkingSteadiness percent samples:", error, nil)]);
            return;
        }
    }];
}
...

{
    ...
    HKCategoryType *type = [HKCategoryType categoryTypeForIdentifier: HKCategoryTypeIdentifierAppleWalkingSteadinessEvent];
    NSPredicate *predicate = [RCTAppleHealthKit predicateForSamplesBetweenDates:startDate endDate:endDate];
    ...
    HKSampleQuery *query = [[HKSampleQuery alloc]
        ...
        resultsHandler:^(HKSampleQuery *query, NSArray *results, NSError *error) {
            ...
            for (HKQuantitySample *sample in results) {
            NSLog(@"sample for AppleWalkingSteadinessEvent %@", sample);
            NSString *startDateString = [RCTAppleHealthKit buildISO8601StringFromDate:sample.startDate];
            NSString *endDateString = [RCTAppleHealthKit buildISO8601StringFromDate:sample.endDate];
...
{
    HKQuantityType *SixMinuteWalkTestDistance = [HKQuantityType quantityTypeForIdentifier:HKQuantityTypeIdentifierSixMinuteWalkTestDistance];

    HKUnit *unit = [HKUnit meterUnit]; // double check this
    ...
    [self fetchQuantitySamplesOfType:SixMinuteWalkTestDistance //
                                unit:unit
                           predicate:predicate
                           ascending:ascending
                               limit:limit
        ...
        if(results){
            ...
            NSLog(@"error getting SixMinuteWalkTestDistance meter samples: %@", error);
            callback(@[RCTMakeError(@"error getting SixMinuteWalkTestDistance meter samples:", error, nil)]);
            return;
        }
    }];
}
...
{
    HKQuantityType *WalkingSpeed = [HKQuantityType quantityTypeForIdentifier:HKQuantityTypeIdentifierWalkingSpeed];

    HKUnit *count = [HKUnit meterUnit];
    HKUnit *second = [HKUnit secondUnit];

    HKUnit *unit = [RCTAppleHealthKit hkUnitFromOptions:input key:@"unit" withDefault:[count unitDividedByUnit:second]];
    ...
    [self fetchQuantitySamplesOfType:WalkingSpeed //
                                unit:unit
                           predicate:predicate
                           ascending:ascending
                               limit:limit
...
{
    HKQuantityType *WalkingStepLength = [HKQuantityType quantityTypeForIdentifier:HKQuantityTypeIdentifierWalkingStepLength];
    ...
    [self fetchQuantitySamplesOfType:WalkingStepLength //
                                unit:unit
                           predicate:predicate
                           ascending:ascending
                               limit:limit
        ...
        if(results){
            ...
            NSLog(@"error getting WalkingStepLength meter samples: %@", error);
            callback(@[RCTMakeError(@"error getting WalkingStepLength meter samples:", error, nil)]);
            return;
        }
    }];
}
...

{
    ...
    [self fetchQuantitySamplesOfType:WalkingAsymmetryPercentage //
        ...
        if(results){
            ...
            NSLog(@"error getting WalkingAsymmetryPercentage percent samples: %@", error);
            callback(@[RCTMakeError(@"error getting WalkingAsymmetryPercentage percent samples:", error, nil)]);
            return;
        }
    }];
}
...
{
    HKQuantityType *QuantityType = [HKQuantityType quantityTypeForIdentifier:HKQuantityTypeIdentifierWalkingDoubleSupportPercentage];
    ...
    [self fetchQuantitySamplesOfType: QuantityType //
        ...
        if(results){
            ...
            NSLog(@"error getting WalkingDoubleSupportPercentage percent samples: %@", error);
            callback(@[RCTMakeError(@"error getting WalkingDoubleSupportPercentage percent samples:", error, nil)]);
            return;
        }
    }];
}
...
{
    HKQuantityType *QuantityType = [HKQuantityType quantityTypeForIdentifier:HKQuantityTypeIdentifierStairAscentSpeed];
    ...
    [self fetchQuantitySamplesOfType: QuantityType //
        ...
        if(results){
            ...
            NSLog(@"error getting StairAscentSpeed samples: %@", error);
            callback(@[RCTMakeError(@"error getting StairAscentSpeed samples:", error, nil)]);
            return;
        }
    }];
}
...
{
    HKQuantityType *QuantityType = [HKQuantityType quantityTypeForIdentifier:HKQuantityTypeIdentifierStairDescentSpeed];
    ...
    [self fetchQuantitySamplesOfType: QuantityType //
        ...
        if(results){
            ...
            NSLog(@"error getting StairDecentSpeed samples: %@", error);
            callback(@[RCTMakeError(@"error getting StairDecentSpeed samples:", error, nil)]);
            return;
        }
    }];
}
