...
RCT_EXPORT_METHOD(getAppleWalkingSteadiness:(NSDictionary *)input callback:(RCTResponseSenderBlock)callback)
{
    [self _initializeHealthStore];
    [self vitals_getAppleWalkingSteadiness:input callback:callback];
}

RCT_EXPORT_METHOD(getAppleWalkingSteadinessEvent:(NSDictionary *)input callback:(RCTResponseSenderBlock)callback)
{
    [self _initializeHealthStore];
    [self vitals_getAppleWalkingSteadinessEvent:input callback:callback];
}

RCT_EXPORT_METHOD(getSixMinuteWalkTestDistance:(NSDictionary *)input callback:(RCTResponseSenderBlock)callback)
{
    [self _initializeHealthStore];
    [self vitals_getSixMinuteWalkTestDistance:input callback:callback];
}

RCT_EXPORT_METHOD(getWalkingSpeed:(NSDictionary *)input callback:(RCTResponseSenderBlock)callback)
{
    [self _initializeHealthStore];
    [self vitals_getWalkingSpeed:input callback:callback];
}
...
{
    [self _initializeHealthStore];
    [self vitals_getWalkingStepLength:input callback:callback];
}

RCT_EXPORT_METHOD(getWalkingAsymmetryPercentage:(NSDictionary *)input callback:(RCTResponseSenderBlock)callback)
{
    [self _initializeHealthStore];
    [self vitals_getWalkingAsymmetryPercentage:input callback:callback];
}

RCT_EXPORT_METHOD(getWalkingDoubleSupportPercentage:(NSDictionary *)input callback:(RCTResponseSenderBlock)callback)
{
    [self _initializeHealthStore];
    [self vitals_getWalkingDoubleSupportPercentage:input callback:callback];
}


RCT_EXPORT_METHOD(getStairAscentSpeed:(NSDictionary *)input callback:(RCTResponseSenderBlock)callback)
{
    [self _initializeHealthStore];
    [self vitals_getStairAscentSpeed:input callback:callback];
}

RCT_EXPORT_METHOD(getStairDescentSpeed:(NSDictionary *)input callback:(RCTResponseSenderBlock)callback)
{
    [self _initializeHealthStore];
    [self vitals_getStairDescentSpeed:input callback:callback];
}
...
- (NSArray<NSString *> *)supportedEvents { // Update
    NSArray *types = @[
        @"ActiveEnergyBurned",
        @"BasalEnergyBurned",
        @"Cycling",
...
- (void)initializeBackgroundObservers:(RCTBridge *)bridge
{
    ...
    if ([HKHealthStore isHealthDataAvailable]) { //update
